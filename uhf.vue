        ...
        created() {
            axios.get(`https://uhf.microsoft.com/en-us/shell/xml/${this.partnerID}?headerid=${this.headerID}&footerid=${this.footerID}&CookieComplianceEnabled=${this.cookieCompliance}`)
                .then(response => {
                    ...
                    const uhfElements = document.querySelectorAll('.uhf');

                    // Hide header and footer
                    uhfElements[0].style.display = 'none';
                    uhfElements[1].style.display = 'none';

                    setTimeout(() => {
                        // Show header and footer
                        uhfElements[0].style.display = 'block';
                        uhfElements[1].style.display = 'block';
